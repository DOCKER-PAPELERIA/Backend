...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 11-06-2024 a las 00:48:25
...
DROP PROCEDURE IF EXISTS `SP_EDITAR_DETALLE`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_DETALLE` (IN `_idDetalle` INT(10), IN `_cantidad` INT(50))   BEGIN
UPDATE detalle
SET cantidad =
CASE
   WHEN _cantidad = '' THEN cantidad
   ELSE _cantidad
END
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_FACTURA` (IN `_idFactura` INT(10), IN `_fecha` DATE)   BEGIN
UPDATE factura
SET fecha =
CASE
...
DROP PROCEDURE IF EXISTS `SP_EDITAR_LOGIN`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_LOGIN` (IN `_idLogin` INT(10), IN `_usuario` VARCHAR(100), IN `_contrasena` VARCHAR(100))   BEGIN
UPDATE login
SET usuario =
CASE
    WHEN _usuario = '' THEN usuario
    ELSE _usuario
END,
   contrasena =
CASE
    ...
    ELSE _contrasena
END
WHERE idLogin = _idLogin;

END$$

DROP PROCEDURE IF EXISTS `SP_EDITAR_METODOPAGO`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_METODOPAGO` (IN `_idMetodoPago` INT(10), IN `_tipopago` VARCHAR(100))   BEGIN
UPDATE metodopago
SET tipopago =
CASE
    WHEN _tipopago = '' THEN tipopago
    ELSE _tipopago
END
...
DROP PROCEDURE IF EXISTS `SP_EDITAR_REGLOGIN`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_REGLOGIN` (IN `_idRegLogin` INT(10), IN `_token` INT(50), IN `_fecha` DATE, IN `_estado` ENUM('activo','inactivo'))   BEGIN
UPDATE reglogin
SET token =
CASE
   WHEN _token = '' THEN token
   ELSE _token
END,
   fecha =
CASE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_ROL` (IN `_idRol` INT(10), IN `_rol` VARCHAR(50))   BEGIN
UPDATE rol
SET rol =
CASE
   WHEN _rol = '' THEN rol
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ELIMINAR_DETALLE` (IN `_idDetalle` INT(10))   BEGIN
DELETE FROM detalle
WHERE idDetalle = _idDetalle;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ELIMINAR_LOGIN` (IN `_idLogin` INT(10))   BEGIN
DELETE FROM login
WHERE idLogin = _idLogin;
END$$

DROP PROCEDURE IF EXISTS `SP_ELIMINAR_METODOPAGO`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ELIMINAR_METODOPAGO` (IN `_idMetodoPago` INT(10))   BEGIN
...
DROP PROCEDURE IF EXISTS `SP_ELIMINAR_REGLOGIN`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ELIMINAR_REGLOGIN` (IN `_idRegLogin` INT(10))   BEGIN
DELETE FROM reglogin
WHERE idRegLogin = _idRegLogin;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ELIMINAR_ROL` (IN `_idRol` INT(10))   BEGIN
DELETE FROM rol
WHERE idRol  = _idRol ;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_CREARCUENTA` (IN `IDUSUARIO` INT(10), IN `IDROL` INT(10), IN `CORREO` VARCHAR(100), IN `CONTRASENA` VARCHAR(100), IN `CONFIRMACION` VARCHAR(100), IN `ESTADO` VARCHAR(50))   BEGIN
INSERT INTO crearcuenta (idUsuario, idRol, correo, contrasena, confirmacion, estado)
VALUES (idUsuario, idRol, correo, contrasena, confirmacion, estado);
END$$

DROP PROCEDURE IF EXISTS `SP_INSERTAR_DETALLE`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_DETALLE` (IN `idFactura` INT(10), IN `idRegProducto ` INT(10), IN `cantidad` INT(50))   BEGIN
INSERT INTO detalle (idFactura, idRegProducto, cantidad)
VALUES (@p0, @p1, @p2);
END$$

DROP PROCEDURE IF EXISTS `SP_INSERTAR_FACTURA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_FACTURA` (IN `idCrearCuenta` INT(10), IN `fecha` DATE)   BEGIN
INSERT INTO FACTURA (idCrearCuenta, fecha)
VALUES (@p0, @p1);
END$$

DROP PROCEDURE IF EXISTS `SP_INSERTAR_LOGIN`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_LOGIN` (IN `idCrearCuenta ` INT(10), IN `usuario` VARCHAR(100), IN `contrasena` VARCHAR(100))   BEGIN
INSERT INTO LOGIN (idCrearCuenta, usuario, contrasena)
VALUES (@P0, @P1, @P2);
END$$

DROP PROCEDURE IF EXISTS `SP_INSERTAR_METODOPAGO`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_METODOPAGO` (IN `idFactura ` INT(10), IN `tipopago` VARCHAR(100))   BEGIN
INSERT INTO metodopago (idFactura, tipopago)
VALUES (@p0, @p1);
END$$
...
DROP PROCEDURE IF EXISTS `SP_INSERTAR_REGLOGIN`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_REGLOGIN` (IN `idLogin ` INT(10), IN `token` INT(50), IN `fecha` DATE, IN `estado` ENUM('activo','inactivo'))   BEGIN
INSERT INTO REGLOGIN (idLogin, token, fecha, estado)
VALUES (@p0, @p1, @p2, @p3);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERTAR_ROL` (IN `ROL` VARCHAR(50))   BEGIN
INSERT INTO rol (ROL)
VALUES (rol);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MOSTRAR_FACTURA` ()   BEGIN
 SELECT idFactura AS ID, idCrearCuenta AS CUENTA, fecha AS FECHA FROM factura;
...
(1, 1, 1, 'juan@example.com', 'contraseña123', 'contraseña123', 'activo'),
(2, 2, 2, 'maria@example.com', 'password456', 'password456', 'activo'),
(4, 4, 2, 'laura@example.com', 'abcde', 'abcde', 'activo'),
(5, 5, 2, 'ana@example.com', 'qwerty', 'qwerty', 'activo'),
(6, 6, 2, 'carocastrillon@gmail.com', 'Caro1234', 'qwerty123', 'activo'),
(7, 7, 2, 'sara@example.com', 'password123', 'password123', 'activo'),
(8, 8, 2, 'jorge@example.com', 'abcdef', 'abcdef', 'activo'),
(62, 2, 2, 'lososj@gagagmail.com', '123', '123', 'activo'),
(69, 1, 1, 'lososj@gagagmail.com', '123', '123', 'activo'),
(70, 1, 1, 'lososj@gagagmail.com', '123', '123', 'activo'),
(74, 4, 2, 'g', '$2b$04$5jae6pOg.NChNhSrjPq0nuRGXG4QlNg.viU4z3ApIri/BOeIyHrN6', '$2b$04$Ay0Ou6DgZnvmLPA8T9ob7uJorC/EUfK2OAZfA5kelqv5mKgSrQINm', 'activo'),
...
INSERT INTO `detalle` (`idDetalle`, `idFactura`, `idRegProducto`, `cantidad`) VALUES
(1, 1, 1, 5),
(2, 2, 2, 3),
(4, 4, 4, 10),
(5, 5, 5, 2),
...
INSERT INTO `factura` (`idFactura`, `idCrearCuenta`, `fecha`) VALUES
(1, 1, '2023-01-01'),
(2, 2, '2023-01-02'),
(4, 4, '2023-01-04'),
(5, 5, '2023-01-05'),
...
(1, 1, 'juanperez', 'contraseña123'),
(2, 2, 'marialopez', 'password456'),
(4, 4, 'laurita', 'abcde'),
(5, 5, 'Laura12', 'aeiou'),
(6, 6, 'carlos123', 'qwerty123'),
(7, 7, 'sara123', 'password123'),
(8, 8, 'jorge123', 'abcdef'),
(11, 7, 'Laura', '123456'),
(51, 62, 'carlos carlo', '123'),
(58, 69, 'Juan Pérez', '123'),
(59, 70, 'Juan Pérez', '123'),
...
INSERT INTO `metodopago` (`idMetodoPago`, `idFactura`, `tipopago`) VALUES
(1, 1, 'Transferencia'),
(2, 2, 'Efectivo');
...
(11, 'Carpeta de cartón color café ', 14, 900, 1700),
(12, 'Cajas de cartón ', 0, 1500, 2200),
(13, 'Marcadores', 13, 7300, 15000);
...
--
-- Volcado de datos para la tabla `reglogin`
--

INSERT INTO `reglogin` (`idRegLogin`, `idLogin`, `fecha`, `estado`) VALUES
(1, 1, '2023-01-01', 'activo'),
(2, 2, '2023-01-02', 'activo'),
(4, 4, '2023-01-04', 'activo'),
(5, 5, '2023-01-05', 'inactivo'),
(6, 6, '2023-01-06', 'activo'),
(7, 7, '2023-01-07', 'activo'),
(8, 8, '2023-01-08', 'inactivo'),
(12, 1, '2024-05-28', 'activo');
...
--
ALTER TABLE `categorias`
  MODIFY `idCategorias` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=22;

--
-- AUTO_INCREMENT de la tabla `crearcuenta`
--
ALTER TABLE `crearcuenta`
  MODIFY `idCrearCuenta` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=80;
...
--
ALTER TABLE `login`
  MODIFY `idLogin` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=68;
...
--
ALTER TABLE `proveedor`
  MODIFY `idProveedor` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `regproducto`
  MODIFY `idRegProducto` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
