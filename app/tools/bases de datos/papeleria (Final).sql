...
-- Tiempo de generación: 17-07-2024 a las 15:00:45
...
DROP PROCEDURE IF EXISTS `SP_CREAR_HISTORIAL2`$$
CREATE DEFINER=`mi_angel123`@`%` PROCEDURE `SP_CREAR_HISTORIAL2` (IN `_nombres` VARCHAR(250), IN `_nombreProducto` VARCHAR(250), IN `_tipopago` VARCHAR(250), IN `_cantidad` INT(100), IN `_fecha` DATE)   BEGIN
    DECLARE _idUsuario INT;
    DECLARE _idProducto INT;
    DECLARE _idMetodoPago INT;

    -- Obtener el idUsuario a partir del nombre
    SELECT idUsuario INTO _idUsuario FROM usuario WHERE nombres = _nombres;
    
    -- Obtener el idProducto a partir del nombre
    SELECT idProducto INTO _idProducto FROM producto WHERE nombre_product = _nombreProducto;

    -- Obtener el idMetodoPago a partir del tipopago
    SELECT idMetodoPago INTO _idMetodoPago FROM metodopago WHERE tipopago = _tipopago;
...
CREATE DEFINER=`mi_angel123`@`%` PROCEDURE `SP_ELIMINAR_HISTORIAL` (IN `_idFactura` INT(10), IN `_correo` VARCHAR(100))   BEGIN
    
    -- Verificamos si el correo y la contraseña coinciden
    IF EXISTS (SELECT * FROM usuario WHERE correo = _correo) THEN
        -- Si el correo y la contraseña son correctos, eliminamos la factura
...
CREATE DEFINER=`mi_angel123`@`%` PROCEDURE `SP_LISTAR_HISTORIAL` ()   BEGIN
    SELECT 
        f.idFactura AS ID,
...
CREATE DEFINER=`mi_angel123`@`%` PROCEDURE `SP_MOSTRAR_HISTORIAL` (IN `_idFactura` INT(10))   BEGIN
    SELECT 
         f.idFactura AS ID,
        p.nombre_product AS Nombre_Producto,
        u.nombres AS Usuario,
...
DROP PROCEDURE IF EXISTS `SP_MOSTRAR_METODOPAGO`$$
CREATE DEFINER=`mi_angel123`@`%` PROCEDURE `SP_MOSTRAR_METODOPAGO` ()   BEGIN
...
(27, 'pablito perez', 'asdfasdfasdf', 'https://www.aciprensa.com/imagespp/sanlucasevangelista.jpg', '2024-07-16'),
(28, 'hjhlkjhlkjhlkjhlkjhkljhlkhlkjh', '12345649874654321654987462321684', 'https://www.aciprensa.com/imagespp/sanlucasevangelista.jpg', '2024-07-16');
...
(3, 1, 14, 'san lucas', -7144, 2135468794, 'https://www.aciprensa.com/imagespp/sanlucasevangelista.jpg', 5000, '2024-07-05', 'activo'),
(22, 1, 1, 'pepe', 195, 2147483647, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQeB7ccJX8-082BMgA_ueorNcTlEEU2PAL5lw&s', 14000, '2024-06-25', 'activo'),
(23, 1, 1, 'Bolígrafo Bic Cristal', -10, 1231313, 'https://res.klook.com/image/upload/c_fill,w_750,h_500/q_80/w_80,x_15,y_15,g_south_west,l_Klook_water_br_trans_yhcmh3/activities/tsah7c9evnal289z5fig.jpg', 20000, '2024-06-25', 'disponible'),
...
(40, 1, 8, 'santiago', 400, 123, 'https://todoenartes.co/12810-large_default/tinta-china-pelikan-rojo-15-gr.jpg', 100000, '2024-07-15', 'activo');
...
(47, 1, 1004738308, 'pablito', '2147483647', '2024-07-15', 'leonardo.franco.tangarife@gmail.com', '$2b$04$Q85Nitjw9GmDoFNZmUw1pOPKl1FdXycmAZjRTLZXf.Z.LhCBnmBUa', 'Activo');
...
--
ALTER TABLE `categorias`
  MODIFY `idCategorias` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=29;
...
--
ALTER TABLE `producto`
  MODIFY `idProducto` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=41;
