...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 26-06-2024 a las 06:24:57
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CREAR_FACTURA` (IN `_idUsuario` INT(10), IN `_idProducto` INT(10), IN `_idMetodoPago` INT(10), IN `_cantidad` INT(100), IN `_fecha` DATE)   BEGIN
    INSERT INTO factura (
        idUsuario, 
        idProducto, 
        idMetodoPago, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_FACTURA` (IN `_idFactura` INT(10), IN `_idUsuario` INT(10), IN `_idProducto` INT(10), IN `_idMetodoPago` INT(10), IN `_cantidad` INT(100), IN `_fecha` DATE)   BEGIN
    UPDATE factura
    SET 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_PRODUCTO` (IN `_idProducto` INT(10), IN `_idCategorias` INT(10), IN `_idProveedor` INT(10), IN `_nombre_product` VARCHAR(200), IN `_stock` INT(25), IN `_codigo_producto` INT(50), IN `_imagen` TEXT, IN `_precio` INT(20), IN `_fecha` DATE, IN `_estado` VARCHAR(50))   BEGIN
UPDATE producto
SET idCategorias =
CASE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_EDITAR_USUARIO` (IN `_idUsuario` INT(10), IN `_idRol` INT(10), IN `_identificacion` VARCHAR(15), IN `_nombres` VARCHAR(50), IN `_telefono` INT(15), IN `_fecha_naci` DATE, IN `_correo` VARCHAR(100), IN `_contrasena` VARCHAR(500), IN `_estado` VARCHAR(50))   BEGIN
UPDATE usuario
SET idRol =
CASE
   WHEN _idRol = '' THEN idRol
   ELSE _idRol
END,
   identificacion =
CASE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_FILTRAR_PRODUCTOS_CATEGORIAS` (IN `_filtro` VARCHAR(50), IN `_categoria` VARCHAR(50))   BEGIN

    DECLARE _idCategoria int;
    SELECT idCategorias INTO _idCategoria FROM categorias WHERE categoria = _categoria;
    
    IF(_filtro = "sin filtro") THEN
    	SELECT * FROM producto WHERE idCategorias = _idCategoria;
    ELSEIF(_filtro = "nuevo") THEN
    	SELECT * FROM producto WHERE idCategorias = _idCategoria ORDER BY fecha DESC;
    ELSEIF(_filtro = "viejo") THEN 
    	SELECT * FROM producto WHERE idCategorias = _idCategoria ORDER BY fecha ASC;
    ELSEIF(_filtro = "ordenar") THEN
    	SELECT * FROM producto WHERE idCategorias = _idCategoria ORDER by nombre_product ASC;
    END IF;
...
SELECT P.idProducto, P.nombre_product, P.stock
FROM producto P
WHERE stock <= 0;
END$$
...
(1, 'Papelería Escolar', 'dededededededed ', 'sdsdsdsdsdsdsdsd', '0000-00-00'),
...
(14, 1, 1, 1, 34, '2024-06-14'),
(20, 1, 1, 1, 5, '2024-06-25'),
(21, 1, 1, 1, 235, '2024-05-25');
...
(1, 1, 3, 'Lapicero azul ', 100, 44, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQeB7ccJX8-082BMgA_ueorNcTlEEU2PAL5lw&s', 1500, '2024-06-12', 'disponible'),
(2, 2, 2, 'Cuaderno argollado', 50, 43, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ02HbBhR9Mf6bNmoJnsuHn2iAfdtQuoaCv2w&s', 3000, '2024-06-10', 'disponible'),
(3, 3, 5, 'Colores', 20, 99933, 'https://papeleriadelahorro.mx/cdn/shop/products/1594004.jpg?v=1631197613&width=500', 5500, '2024-06-10', 'disponible'),
(4, 4, 7, 'Borrador', 0, 34343434, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBIj-rXWwx-VNC63wK-vQJgGI3u7Op2-gtCAF0LtQsA_H3FaaYQkh-7f_eRv0nF5fTPJ8&usqp=CAU', 700, '2024-06-10', 'disponible'),
(5, 1, 1, 'marcadores', 0, 2147483647, 'https://www.google.com/imgres?q=imagenes%20de%20productos%20de%20papeleria&imgurl=https%3A%2F%2Fwww.panafargo.com%2Fwp-content%2Fuploads%2F2018%2F08%2F50-arti%25CC%2581culos-de-papeleria-Karin.jpg&imgrefurl=https%3A%2F%2Fpanafargo.com%2F50-articulos-de-papeleria%2F&docid=VWxY170qa1HawM&tbnid=CMM8MNXuBaDlvM&vet=12ahUKEwiazc_7k-aGAxWwq4QIHdYaCqw4ChAzegQIFhAA..i&w=980&h=1139&hcb=2&ved=2ahUKEwiazc_7k-aGAxWwq4QIHdYaCqw4ChAzegQIFhAA', 12000, '2024-06-25', 'disponible'),
(22, 1, 1, 'cuadernos', 200, 2147483647, 'https://www.google.com/imgres?q=imagenes%20de%20productos%20de%20papeleria&imgurl=https%3A%2F%2Fwww.panafargo.com%2Fwp-content%2Fuploads%2F2018%2F08%2F50-arti%25CC%2581culos-de-papeleria-Karin.jpg&imgrefurl=https%3A%2F%2Fpanafargo.com%2F50-articulos-de-papeleria%2F&docid=VWxY170qa1HawM&tbnid=CMM8MNXuBaDlvM&vet=12ahUKEwiazc_7k-aGAxWwq4QIHdYaCqw4ChAzegQIFhAA..i&w=980&h=1139&hcb=2&ved=2ahUKEwiazc_7k-aGAxWwq4QIHdYaCqw4ChAzegQIFhAA', 14000, '2024-06-25', 'disponible'),
(23, 1, 1, 'Bolígrafo Bic Cristal', 222, 1231313, 'null', 20000, '2024-06-25', 'disponible'),
(24, 1, 2, 'Lápiz HB Faber-Castell\r\n', 123, 12134342, 'null', 2333, '2024-06-25', 'disponible'),
(25, 2, 5, 'Cuaderno de espiral A4 Oxford', 333, 6765345, 'null', 59000, '2024-06-25', 'disponible'),
(26, 1, 4, 'Archivador de palanca Esselte', 66, 87654567, 'null', 300000, '2024-06-25', 'disponible'),
(27, 1, 7, 'Resaltadores Stabilo Boss', 555, 343532, 'null', 2000000, '2024-06-25', 'disponible'),
(28, 2, 4, 'Bloc de notas adhesivas Post-it', 333, 987654321, 'null', 200000, '2024-06-25', 'disponible'),
(29, 2, 5, 'Cartulina tamaño carta', 222, 123456789, 'null', 200000, '2024-06-25', 'disponible'),
(30, 2, 2, '2', 2, 2, '2', 2, '2024-06-25', '2');
...
(1, 1, 12313131, 'carlos gomez', '12345678', '2004-02-17', 'lodjnbd@gmail.com', '1234', 'activos'),
...
(11, 1, 1040516846, 'Carolina', '315498244', '2005-11-28', 'pi', '22', 'activo'),
(12, 1, 123456874, 'Lisandro', '3204879654', '2005-04-17', 'pu', '33', 'activo'),
(16, 1, 122224, 'carl', '12367', '2041-09-15', 'py', '44', 'activo'),
(17, 1, 12222224, 'corls', '1222367', '2041-09-15', 'qw', '55', 'activo');
...
ALTER TABLE `factura`
  ...
  ADD KEY `Factura_ibfk_3` (`idProducto`);
...
ALTER TABLE `proveedor`
  ...
  ADD UNIQUE KEY `nombre_proveedor` (`nombre_proveedor`,`telefono`,`correo`);
...
ALTER TABLE `usuario`
  ...
  ADD UNIQUE KEY `identificacion` (`identificacion`,`nombres`,`fecha_naci`,`correo`,`contrasena`),
  ADD KEY `idRol` (`idRol`);
...
--
ALTER TABLE `factura`
  MODIFY `idFactura` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=22;
...
--
ALTER TABLE `producto`
  MODIFY `idProducto` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=32;
...
--
ALTER TABLE `usuario`
  MODIFY `idUsuario` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;
...
--
ALTER TABLE `factura`
  ADD CONSTRAINT `Factura_ibfk_1` FOREIGN KEY (`idMetodoPago`) REFERENCES `metodopago` (`idMetodoPago`) ON DELETE NO ACTION ON UPDATE CASCADE,
  ADD CONSTRAINT `Factura_ibfk_2` FOREIGN KEY (`idUsuario`) REFERENCES `usuario` (`idUsuario`) ON DELETE NO ACTION ON UPDATE CASCADE,
  ADD CONSTRAINT `Factura_ibfk_3` FOREIGN KEY (`idProducto`) REFERENCES `producto` (`idProducto`) ON DELETE NO ACTION;

--
-- Filtros para la tabla `producto`
--
ALTER TABLE `producto`
  ADD CONSTRAINT `fk_idcategoria_producto` FOREIGN KEY (`idCategorias`) REFERENCES `categorias` (`idCategorias`) ON DELETE NO ACTION ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_idproveedor_producto` FOREIGN KEY (`idProveedor`) REFERENCES `proveedor` (`idProveedor`) ON DELETE NO ACTION ON UPDATE CASCADE;
